...
class FeedbackWebhookHandler:
    ...
    def parse_github_feedback(self, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    def parse_gitlab_feedback(self, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    def parse_bitbucket_feedback(self, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        ...
        if event_type == 'pullrequest:comment_created':
            ...
            if emojis:
                ...
                return {
                    ...
                    'comment_body': body,
                    'repo_owner': payload.get('repository', {}).get('owner', {}).get('username', ''),
                    'repo_name': payload.get('repository', {}).get('name', ''),
...
async def github_feedback_webhook(
    ...
    try:
        ...
        logger.info(f"Received GitHub feedback: {feedback.get('event_type')} from {feedback.get('user')}")
...
async def gitlab_feedback_webhook(
    ...
    try:
        ...
        logger.info(f"Received GitLab feedback: {feedback.get('event_type')} from {feedback.get('user')}")
...
async def bitbucket_feedback_webhook(
    ...
    try:
        ...
        logger.info(f"Received Bitbucket feedback: {feedback.get('event_type')} from {feedback.get('user')}")
