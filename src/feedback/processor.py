...
from feedback.classifier import EmojiClassifier, ClassificationResult, FeedbackType
...
class FeedbackProcessor:
    ...
    def __init__(
        ...
        classifier: Optional[EmojiClassifier] = None
    ):
        """
    ...
    async def _store_feedback(self, record: Dict[str, Any]):
        ...
        try:
            ...
            await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: doc_ref.set(record_copy)
            )
    ...
    async def _find_review_id(self, record: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            query = (
                reviews_ref
                .where('repo_owner', '==', record['repo_owner'])
    ...
    async def get_feedback_summary(
        ...
        try:
            ...
            total = len(feedbacks)
            positive = sum(1 for f in feedbacks if f.get('feedback_type') == FeedbackType.POSITIVE.value)
            negative = sum(1 for f in feedbacks if f.get('feedback_type') == FeedbackType.NEGATIVE.value)
            neutral = sum(1 for f in feedbacks if f.get('feedback_type') == FeedbackType.NEUTRAL.value)
            confused = sum(1 for f in feedbacks if f.get('feedback_type') == FeedbackType.CONFUSED.value)
