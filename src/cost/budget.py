...
class BudgetConfig:
    ...
    # Per-repo budgets
    repo_daily_budgets: Dict[str, float] = None
...
class BudgetEnforcer:
    ...
    async def check_daily_budget(self, repo: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if repo and repo in self.config.repo_daily_budgets:
            budget_limit = self.config.repo_daily_budgets[repo]
        else:
    ...
    async def can_review_pr(
        ...
        can_proceed = (
            daily_status['can_proceed'] and
            pr_status['can_proceed'] and
            monthly_status['can_proceed']
        )
    ...
    async def _get_daily_spend(self, repo: Optional[str] = None) -> float:
        ...
        try:
            ...
            query = (
                ...
                .where('timestamp', '<', tomorrow.isoformat())
            )
