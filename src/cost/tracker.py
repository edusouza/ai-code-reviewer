...
class CostTracker:
    ...
    async def get_daily_cost(self, date: Optional[datetime] = None) -> float:
        ...
        try:
            ...
            query = (
                costs_ref
                .where('timestamp', '>=', start_of_day.isoformat())
                .where('timestamp', '<=', end_of_day.isoformat())
            )
    ...
    async def get_repo_cost_summary(
        self,
        repo: str,
        days: int = 30
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for r in records:
                ...
                by_model[model]['tokens'] += r.get('prompt_tokens', 0) + r.get('completion_tokens', 0)
            ...
            return {
                ...
                'average_cost_per_review': round(total_cost / len(full_reviews), 4) if full_reviews else 0,
                'by_model': by_model
            }
...
def init_cost_tracker(firestore_db: Optional[Any] = None) -> CostTracker:
    """
