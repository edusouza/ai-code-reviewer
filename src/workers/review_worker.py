...
class ReviewWorker:
    ...
    def __init__(
        ...
        # Processing callback
        self._process_callback: Optional[Callable[[ReviewJob], Any]] = None
    ...
    def start(self):
        ...
        flow_control = pubsub_v1.types.FlowControl(
            max_messages=self.max_workers * 2
        )
...
_worker: Optional[ReviewWorker] = None
...
def get_worker() -> Optional[ReviewWorker]:
...
async def process_review_job(job: ReviewJob):
    ...
    async def process_review_job(job: ReviewJob):
        ...
    """
    from graph.builder import create_review_graph
    from graph.state import ReviewState, ReviewConfig, ReviewMetadata
    from observability.langfuse_client import get_langfuse
    from observability.decorators import trace_workflow
