...
from observability.langfuse_client import get_langfuse, current_trace_id, current_span_id
...
def trace_agent(name: Optional[str] = None, agent_type: Optional[str] = None):
    """
...
def trace_llm(model_name: Optional[str] = None):
    """
...
def _extract_metadata(func: Callable, args: tuple, kwargs: dict) -> Dict[str, Any]:
    ...
    if args and hasattr(args[0], 'provider'):
        ...
        metadata['repo'] = f"{getattr(obj, 'repo_owner', '')}/{getattr(obj, 'repo_name', '')}"
        metadata['pr_number'] = getattr(obj, 'pr_number', None)
    
    return metadata


def _extract_input_data(args: tuple, kwargs: dict) -> Optional[Dict[str, Any]]:
...
def _extract_token_usage(result: Any) -> Dict[str, Optional[int]]:
